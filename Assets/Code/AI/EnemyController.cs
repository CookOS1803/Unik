...
public class EnemyController : MonoBehaviour, IMoveable, IMortal
{
    ...
    IEnumerator Patroling()
    {
        ...
        while (!aiManager.alarm)
        {
            ...
            yield return new WaitUntil(() => agent.velocity.sqrMagnitude < 0.01f && Vector3.Distance(agent.destination, transform.position) <= agent.stoppingDistance || aiManager.alarm || isSeeingPlayer);
